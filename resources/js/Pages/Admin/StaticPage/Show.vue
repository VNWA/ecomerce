<template>
    <AppLayout title="Static Pages">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Static Pages</h2>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg pt-8 pb-12 px-2">
                    <div class="float-left">
                        <button :disabled="itemsSelected.length <= 0"
                            ...
                            <Icon icon="material-symbols:close-rounded"   class="mr-1" /> Delete Selection
                        </button>
                    </div>
                    <div class="float-right text-xs uppercase">
                        <Link :href="route('StaticPage.Create')"
                    ...
                    <div class="my-2 py-10">
                        <DataTable :headers="headers" :items="dataPage" buttons-pagination show-index
                            ...
                            <template #item-is_header="{ id, is_header }">
                                <div class="flex items-center cursor-pointer">
                                    <div class="relative" @click="handleIsHeaderChange(id)">
                                        <div class="toggle-path bg-gray-300 w-9 h-5 rounded-full p-0">
                                            <div class="toggle-circle w-5 h-5 rounded-full shadow-md"
                                                :class="{ 'transform translate-x-full bg-purple-500': is_header == 1, 'bg-white': is_header == 0 }">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </template>
                            <template #item-is_header_main="{ id, is_header_main }">
                                <div class="flex items-center cursor-pointer">
                                    <div class="relative" @click="handleIsHeaderMainChange(id)">
                                        <div class="toggle-path bg-gray-300 w-9 h-5 rounded-full p-0">
                                            <div class="toggle-circle w-5 h-5 rounded-full shadow-md"
                                                :class="{ 'transform translate-x-full bg-purple-500': is_header_main == 1, 'bg-white': is_header_main == 0 }">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <template #item-operation="{ id, name }">
                                <div class="py-3 flex items-center justify-center">
                                    ...
                                    <Link :href="route('StaticPage.Edit', id)"
...
const headers = ref([
    ...
    { text: "Is  Header", value: "is_header" },
    { text: "Is Main Header", value: "is_header_main" },
...
const loadData = () => {
    axios.post('/vnwa/static-pages/load-data-table').then((response) => {
...
const changeORD = (id, name, event) => {
    ...
    if (ordValue) {
        axios.post('/vnwa/change-ord', { tb: 'static_pages', id, value: ordValue }).then(() => {
            toast.success(`Update ORD  ${name} thành ${ordValue} thành công!`, { autoClose: 1000 });
        }).catch(error => console.error(error));
    }
};
...
const showModalDeleteMultipleItem = () => {
    const commonItems = dataPage.value.filter(item1 =>
        itemsSelected.value.some(item2 => item1.id === item2.id)
    );
    itemsDelete.value = commonItems
    modalDelete.value = true;
};
...
const deleteItems = () => {
    ...
    axios.post(route('StaticPage.Delete'), body).then(() => {
        loadData();
        itemsSelected.value = [];
        modalDelete.value = false;
...
const searchDataTable = () => {
    axios.post('/vnwa/static-pages/load-data-table', { search_name: searchName.value }).then((response) => {
        dataPage.value = response.data.data;
    }).catch(error => console.error(error));
};

const handleStatusChange = (id, status) => {
    axios.post('/vnwa/change-status', { tb: 'static_pages', id }).then(() => {
        loadData();

        toast.success('Update  Status Success!', { autoClose: 1000 });
    }).catch(error => console.error(error));
};
const handleIsHeaderChange = (id) => {
    axios.post(route('StaticPage.ChangeIsHeader', id)).then((response) => {
        loadData();

        toast.success(response.data.message, { autoClose: 1000 });
    }).catch(error => console.error(error));
};
const handleIsHeaderMainChange = (id) => {
    axios.post(route('StaticPage.ChangeIsHeaderMain', id)).then((response) => {
        loadData();
