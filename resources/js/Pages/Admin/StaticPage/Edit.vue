<template>
    <div>
        <AppLayout title=" Update Static Page" :isLoading="isPageLoading">
            <template #header>
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Update Static Page</h2>
                    </div>
                    <div>
                        <HeaderBreadcrumbs
                            :breadcrumbs="[['StaticPage', route('StaticPage')], [' Create', route('StaticPage.Edit', props.staticPage.id)]]" />
                    </div>
                </div>
            </template>
            ...
            <div class="grid grid-cols-12 gap-4 p-2">
                <div class="lg:col-span-8 col-span-12">
                    <div class="bg-white shadow rounded-sm mb-3 p-2">
                        ...
                        <div class="mb-4">
                            <InputLabel for="desc">Desciption <span class="text-red-500">*</span></InputLabel>
                ...
                <div class="lg:col-span-4 col-span-12">
                    <div class="flex flex-col">
                        ...
                        <div class="bg-white shadow rounded-sm mb-3 p-2 sticky top-0">
                            ...
                            <div class="bg-white shadow rounded-sm mb-3 p-2">
                                <div class="flex items-center ps-4 border border-gray-200 rounded mb-3 ">
                                ...
                                <div class="flex items-center ps-4 border border-gray-200 rounded mb-3 ">
                                    <input type="checkbox" id="is_header_main"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500   focus:ring-2  "
                                        v-model="form.is_header_main" :true-value="1" :false-value="0" />

                                    <label for="is_header_main"
                                        class="w-full py-2 ms-2 text-sm font-medium text-gray-900  cursor-pointer">
                                        Is main header
                                    </label>
                                </div>
...
const form = ref({
    ...
    is_header_main: props.staticPage.is_header_main,
    is_header: props.staticPage.is_header,
    image: props.staticPage.image,
    name: props.staticPage.name,
    slug: props.staticPage.slug,
    desc: props.staticPage.desc,
    content: props.staticPage.content,
    seo_meta: {
        meta_title: props.staticPage.meta_title,
        meta_desc: props.staticPage.meta_desc,
        meta_image: props.staticPage.meta_image,
    }
});
const errors = ref({
    image: '',
...
const checkSlug = (slug) => {
    clearError();
    if (slug) {
        const model_type = 'App\Models\StaticPage';
...
const submit = (isRollBack = true) => {
    ...
    axios.post(route('StaticPage.Update', props.staticPage.id), form.value).then(response => {
