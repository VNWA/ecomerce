<template>
    <div>
        <AppLayout title=" Create New Static Page" :isLoading="isPageLoading">
            <template #header>
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Create New Static Page</h2>
                    </div>
                    <div>
                        <HeaderBreadcrumbs
                            :breadcrumbs="[['StaticPage', route('StaticPage')], [' Create', route('StaticPage.Create')]]" />
                    </div>
                </div>
            </template>
...
const submit = (isRollBack = true) => {
    ...
    axios.post('/vnwa/static-pages/create', form.value).then(response => {
