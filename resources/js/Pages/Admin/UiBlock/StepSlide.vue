                ...
                <div class="mb-4">
                    ...
                    <div class="space-y-2">
                        <div v-for="(step, stepIndex) in formData.steps" :key="stepIndex" class="border p-2 rounded-md">
                            ...
                            <div>
                                <InputLabel :for="'step-desc-' + stepIndex">Step Description</InputLabel>
                                <textarea :id="'step-desc-' + stepIndex" v-model="step.desc"
                                    class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                    rows="5"></textarea>
                            </div>
                            <div>
                                <InputLabel :for="'step-image-' + stepIndex">Step Image URL</InputLabel>
                                <InputUrlImage :id="'step-image-' + stepIndex" v-model="step.image"
                                    placeholder="Step Image URL" class="mt-1 block w-full" />
                            </div>
                            ...
                            <div>
                                ...
                                <Multiselect v-model="step.products" mode="tags" :close-on-select="false"
                                    :searchable="true" :create-option="true" :options="productOptions" />
                            </div>
                            <button @click="removeStep(stepIndex)"
                                class="bg-red-500 hover:bg-red-600 text-white rounded-md px-2 py-1 mt-2">
                                Remove Step
                            </button>
                        </div>
...
const addStep = () => {
    formData.steps.push({
        ...
        link: '',
        image: '',
        products: [] // Danh sách sản phẩm cho bước này
    });
};
...
const handleSubmit = () => {
    ...
    const shortcode = `[StepSlideWithItems {title="${formData.title}" data='${steps.replace(/'/g, "\\'")}'}  /]`
