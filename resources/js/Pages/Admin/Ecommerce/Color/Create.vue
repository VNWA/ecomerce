<template>
    <div>
        <AppLayout title=" Create New Color" :isLoading="isPageLoading">
            <template #header>
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Create New Color</h2>
                    </div>
                    <div>
                        <HeaderBreadcrumbs
                            :breadcrumbs="[['Color', route('Ecommerce.Color')], [' Create', route('Ecommerce.Color.Create')]]" />
                    </div>
                </div>
            </template>
...
const submit = (isRollBack = true) => {
    ...
    axios.post('/vnwa/ecommerce/colors/create', form.value).then(response => {
